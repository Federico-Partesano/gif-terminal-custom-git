...
export const moveBranch = async () => {
  ...
  const listBranch = all
    ...
    .map((branch) =>
      branch.includes("remotes") ? branch.split("/").slice(2).join("/") : branch
    );
    
    updateBottomBarBranches("Branches loaded! \n");
    const enquirerChoiceBranch = new Enquirer<{choice: string}>();

    const { choice } = await enquirerChoiceBranch.prompt([
      {
        type: "autocomplete",
        name: "choice",
